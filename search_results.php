...
$sql = $con->query("SELECT * FROM `posters` WHERE status=1 AND username LIKE '%$searchTerm%' ORDER BY id DESC");
...
<div id="popup-sharePostcontainer_<?php echo $dyn; ?>" class="popup-container" style="display:none;">
    ...
    .form-control:focus::placeholder {
        color: #d63384; /* Adjust this hex code for the desired pink shade */
    }
							</style>
 ...
 function showPopup(useridgettttt,flowdyif) {
	//	debugger;
		...
		$.ajax({
			...
			type: 'POST',
			data: { userid: useridgettttt }, // Pass useriddd to the server
...
function threedotunfollowpopcard(dyidgett, useriddd) {
    ...
    $('#afterfollowingpopcard').show();
	 ...
	 $.ajax({
        ...
        type: 'POST',
        data: { userid: useriddd }, // Pass useriddd to the server
